...
/*A wrapper around ncsa_getline to do rfc822 line unfolding*/
int ht_rfc822_ncsa_getline(char *s,int n,int f,unsigned int timeout)
  {
    static char pb=0;
    int len;
    ...
    while(!ncsa_getline(s,n,f,timeout)){
      len=strlen(s);
      s+=len;      
      n-=len;

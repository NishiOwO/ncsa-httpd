...
void ncsa_getline_timed_out(int sig) 
{
 ...
 */
int ncsa_getline(sock_buf *sb, char *s, int n, int options, unsigned int timeout)
{
    char *endp = s + n - 1;
    int have_alarmed = 0;
...
/*    static char buffer[HUGE_STRING_LEN]; */
	...
	*s = '\0';
	...
	if (sb->buf_posn == sb->buf_good) {
	    have_alarmed = 1;
	    signal(SIGALRM,ncsa_getline_timed_out);
	    alarm(timeout);
...
int cfg_ncsa_getline (char* s, int n, FILE* fp)
{
    int   len = 0, ch;
